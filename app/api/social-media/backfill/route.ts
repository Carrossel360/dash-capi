...
import { backfillWorkspaceInstagram } from '@/lib/social-sync'
import { dateRange } from '@/lib/trafego-period'

const MAX_BACKFILL_DAYS = 90

// Botão "Buscar dados históricos" em Social Media — dispara quando o período selecionado
// (ex: mês anterior) ainda não tem dados porque o sync horário só cobre uma janela móvel curta.
export async function POST(req: NextRequest) {
  ...
  const result = await backfillWorkspaceInstagram(workspace, fromStr, toStr)

...
import { backfillWorkspaceGoogleAds } from '@/lib/ads-sync'
import { dateRange } from '@/lib/trafego-period'

const MAX_BACKFILL_DAYS = 90

// Botão "Buscar dados históricos" em Tráfego Pago (Google Ads) — dispara quando o período
// selecionado (ex: mês anterior) ainda não tem dados completos porque o sync horário só
// reprocessa uma janela móvel de 30 dias (lib/ads-sync.ts syncWindow), e a conta de anúncio
// pode ter sido configurada bem depois de o período em questão já ter "envelhecido" pra fora
// dessa janela.
export async function POST(req: NextRequest) {
  ...
  const result = await backfillWorkspaceGoogleAds(workspace, fromStr, toStr)
